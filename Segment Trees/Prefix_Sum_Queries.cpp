...
void build(int idx, int l, int r) {
    if (l == r) {
        ...
        tree[idx] = {(long long)arr[l], (long long)arr[l]};
        return;
    }
...
void update(int idx, int l, int r, int pos, int x) {
    if (l == r) {
        tree[idx] = {(long long)x, (long long)x};
        return;
    }
...
int main() {
    ...
    while(q--) {
        ...
        if (type == 1) {
            int pos, x;
            cin >> pos >> x;
            update(0, 0, n-1, pos - 1, x); // 1-based to 0-based
        } else {
            int a, b;
            cin >> a >> b;
            Node res = query(0, 0, n-1, a - 1, b - 1); // 1-based to 0-based
